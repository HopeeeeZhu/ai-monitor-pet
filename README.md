...
一个住在 Mac 桌面的 AI 监工：谁还在跑、谁等你授权、谁已经做完，它替你盯着。

同时开很多 AI agent 干活时，脑子根本记不住"哪个在跑、哪个卡住了、哪个跑完了"。小安竺替你记：AI 在跑任务时她跑步，需要授权时举手提醒你，任务跑完跳跃庆祝，没事时安静睡觉。

## 适合这些时候

- 同时开着 Claude、Codex、Qoder 等多个 AI 工具。
- AI 在后台跑很久，常常忘记回来批准或查看结果。
- 想知道某个任务属于哪个工具、项目和会话。
- 临时冒出待办或想法，想用快捷键直接说出来记住。
...
日常操作：左键拖动换位置，左键单击开关面板，右键调大小或退出。按 `Control + Option + V` 开始语音记录，再按一次会停止并自动保存为待办；需要编辑或设置提醒时，点脚下录音面板里的「停止」按钮。左键面板右上角的收纳盒图标进入捕获箱。
...
- **v1.2.1**（2026-07-08）：优化语音录入结束方式，快捷键二次按下自动保存；新增明确停止按钮和手动记录待办入口
