...
claude cli、Aone Copilot (IntelliJ IDE 内)、Qoder (IDE/CLI)、QoderWork、悟空 Wukong、Claude 桌面端、Codex 桌面端。
...
└───────────────▲───────────────────────▲────────────────┘
    ...
    │  准、省、首选          │   │  hook覆盖不到时启用        │
    ├──────────────────────┤   ├──────────────────────────┤
    │ Claude CLI: hooks     │   │ Aone Copilot(IntelliJ):  │
...
配置位置: `~/.claude/settings.json` → hooks 段
...
#### 🟠 Claude / Codex 桌面端 (GUI) — 需 Accessibility
...
|------|---------|------------------|------|
| Claude CLI | ✅ 独立进程 | ✅ `session_id` | hook payload |
...
- **CLI端(Claude/Qoder/Codex)**: 收到 `PreToolUse`/`PostToolUse`（紧跟在 permission_prompt 之后）= 用户授权完，回到 running
...
#### Phase 2（扩展更多工具）
6. 接入 Claude CLI、Codex
7. 接入悟空（前端日志监听）
8. 接入 Claude/Codex 桌面端 GUI（Accessibility）
